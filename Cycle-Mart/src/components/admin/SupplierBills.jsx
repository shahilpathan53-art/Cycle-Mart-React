...
const SupplierBills = () => {
  ...
  const fetchBills = async () => {
    try {
      const res = await fetch("http://localhost:4000/api/admin/supplier-bills");
  ...
  const generateBillId = () => {
    const randomId = "SUP-" + Math.floor(100000 + Math.random() * 900000);
    setNewBill((prev) => ({ ...prev, billId: randomId }));
  };

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!newBill.billId || !newBill.supplierName || !newBill.date || !newBill.totalAmount) {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Failed to add supplier bill");

      const savedBill = await res.json();
      setBills((prev) => [...prev, savedBill]);
      setNewBill({ billId: "", supplierName: "", date: "", totalAmount: "" });
      alert("✅ Supplier bill added successfully!");
    } catch (err) {
      console.error("Error adding supplier bill:", err);
      alert("❌ Failed to add supplier bill.");
    } finally {
      setAdding(false);
    }
  };
      ...
      <div className="container py-5">
        ...
        {loading ? (
          <div className="text-center py-5">
            ...
            ></div>
            <p className="mt-3 fs-5 text-light">Loading supplier bills...</p>
          </div>
        ) : bills.length === 0 ? (
          <p className="text-center text-muted">No supplier bills found 😕</p>
        ) : (
          <div
